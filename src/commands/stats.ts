...
const createStatEmbed = (user: User, name: string, data: UserStats): APIEmbed => {
  return makeEmbed({
    color: 0x00FF00,
    title: `${name}'s stats`,
    author: { name: user.username, icon_url: user.avatarURL() ?? undefined },
    fields: [
    ...
    footer: { text: 'Untrusted Bot', icon_url: config.avatar },
  })
}
...
const stats = async (options: CommandExecuteParameters) => {
  ...
  const username = isSlash ? interaction?.options.get('username')?.value as string : args?.join(' ')
