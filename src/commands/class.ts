...
const skillDescriptionEmbedField = (sk: string) => {
    ...
    return { name: skill!.name, value: skill!.description, inline: false }
}
...
const createClassEmbeds = (cl: UntrustedClass, user: User): MessageEmbed[] => {
    ...
    const classDaySkills: EmbedField[] = cl.day_skills.map(sk => skillDescriptionEmbedField(sk)) ?? []
    const classNightSkills: EmbedField[] = cl.night_skills.map(sk => skillDescriptionEmbedField(sk)) ?? []
    const classPassiveSkills: EmbedField[] = cl.passive_skills.map(sk => skillDescriptionEmbedField(sk)) ?? []
    for (let i = 0; i < 3; i++) embeds.push(makeEmbed({
        color: getClassColor(cl.faction),
        author: { name: cl.ingame_name, iconURL: user.avatarURL() ?? '' },
        thumbnail: { url: getIcon(Number(cl.id), true) },
        footer: { text: `Untrusted Bot | Page ${i + 2} of 4`, iconURL: config.avatar }
    }))
...
const classCommand = async (options: CommandExecuteParameters) => {
    ...
    if (!user) return
    const query = isSlash ? interaction!.options.getString('class') : args?.join(" ")
    const classData = findClass(query)
    if (!classData) return isSlash ? interaction?.reply('No class found') : message?.reply('No class found')
    ...
    let page = 0
    const filter = (i: MessageComponentInteraction) => i.user.id === user.id && i.message.id === reply?.id
...
export default new Command('class', 'Use this command to get information about a certain class.', ['classinfo', 'cl', 'ci'],
    classCommand, options)