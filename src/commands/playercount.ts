...
const playerCount = async (options: CommandExecuteParameters) => {
  ...
  var activeGamesText = `**${playerCount.Active_games} active games.**`;
  ...
  if (Number(playerCount.Active_games) == 1) activeGamesText = "**1 active game.**"
  ...
  */
  const response = `${operativesText}\n${lobbiesText} and ${activeGamesText}`; //Fully formatted.
  return isSlash ? interaction?.reply(response) : message?.reply(response)
}
